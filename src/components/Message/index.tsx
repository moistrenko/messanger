...
const Message: React.FC<IMessageParams> = ({ message }) => {
	const isYouMessage = message.user.you;
	const YouMessageClass = isYouMessage ? "my-message" : "";
	const isNewMessage = message.is_new;
	const messageIcon = (
	...
	return (
		<>
			...
			<div className={`message ${YouMessageClass}`}>
				{!isYouMessage && (
				...
				<div className='message__content content'>
					{!isYouMessage && (
						<h3 className='content__title'>{`${message.user.name} ${message.user.surname}`}</h3>
					)}
					<div className='content__message'>
						<div className='message-text'>{message.message} </div>
						<span className='message-time'>
							{dayjs(message.created_at).format("HH:mm")} {isYouMessage && messageIcon}
						</span>
					</div>
				</div>
			</div>
